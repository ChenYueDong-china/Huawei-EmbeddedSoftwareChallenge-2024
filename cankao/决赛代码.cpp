...
private:
    ...
    //生成随机数序列，虽然种子相同但是，序列得元素不同，每一次运行结果相同
    // ，但是每一次运行迭代每次都是不同数，因为是一个随机数序列相同.
    //保证了可复现。。
    mt19937 engine;
private:
    int cover[MAXP][MAXM];
...
public:
    ...
    pair<int, vector<int>> astar(int start, int terminate, int chan = -1) {
        static int dist[MAXP][MAXN], timestamp[MAXP][MAXN], id = 1;
        id += 1;
        static pair<int, int> parent[MAXP][MAXN];
        static fast_deque<pair<int, int>> A, B1, B2, C;
        A.clear();
        B1.clear();
        B2.clear();
        C.clear();
        ...
        while (A.size() || B1.size() || B2.size() || C.size()) {
            ...
            const auto [state, distance] = A.back();
            A.pop_back();
    ...
    pair<int, vector<int>> bidirectional_bfs(int start, int terminate, int chan = -1) {
        ...
        static int timestamp[MAXP][MAXN], id = 1;
        id += 1;
